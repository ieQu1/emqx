...
do_open_db(DB, RuntimeConf, S0 = #s{dbs = DBs}) ->
    maybe
        ...
        {ok, BUPp} ?= get_backup_order(RuntimeConf),
        S = S0#s{
            dbs = DBs#{
                DB => #dbs{rtconf = RuntimeConf, gvars = GVars, backup_order = BUPp}
            }
        },
        ...
        #{dbs := DBSchema} ->
            L = lists:sort(
                maps:fold(
                    fun(DB, _, Acc) ->
                        maybe
                            #{runtime := #{backup_order := Order}} ?= get_db_runtime(DB),
                            true ?= is_integer(Order),
                            [{Order, DB} | Acc]
                        else
                            _ -> Acc
                        end
...
get_backup_order(#{backup_order := BUPp}) ->
    case is_integer(BUPp) of
        true ->
            {ok, BUPp};
        false ->
            {error, {invalid_backup_order, BUPp}}
    end;
get_backup_order(#{}) ->
    %% Do not backup:
    {ok, false}.
