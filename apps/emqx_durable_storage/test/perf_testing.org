...
%% API:
-export([l/0, create_db/1, test_dbs/0, create_dbs/0, counter_test/1, owned_counter_test/1, append_test/1]).
...
def my_bins(df, q=0.99, n=30):
    return np.linspace(df['Val'].min(), round(df['Val'].quantile(q=q)), n)

# Style of histogram where plots are overlayed
...
def histogram_vs_separate(df, title):
    ...
    df.hist(column='Val', grid=True, by=mtypes, sharex=True, sharey=True, bins=bins, histtype='step', density=True)
    # plt.tight_layout()
    fn = title + ".hist.png"
    plt.savefig(fn)
    return fn
...
def boxplot_vs(df, title,by='DB'):
    plt.close()
    fn = title + ".bp.png"
    df.boxplot(column='Val', by=by, flierprops=dict(marker='.', markersize=2), whis=(0, 99), showfliers=False)
    plt.savefig(fn)
    return fn
#+end_src
...
os.system("docker cp dev-cluster-emqx1-1:/opt/emqx/data/append.csv .")
df_append = pd.read_csv('append.csv', sep=';')
...
df_append = pd.read_csv('../../../_build/emqx-enterprise/rel/emqx/data/append.csv', sep=';')
#+end_src
...
#+begin_src erlang
append_loop(MyId, Opts = #{db := DB, batch_size := BS, payload := Payload}, undefined) ->
  Shard = emqx_ds:shard_of(DB, MyId),
  Topic = [<<"t">>, integer_to_binary(MyId)],
  Batch = [{Topic, ?ds_tx_ts_monotonic, Payload} || _ <- lists:seq(1, BS)],
  State = {Shard, Batch},
  append_loop(MyId, Opts, State);
append_loop(MyId, Opts = #{db := DB}, {Shard, Batch} = State) ->
  ?with_metric(t,
               begin
...
append_test(UserOpts = #{db := DB, payload_size := PSize}) ->
  ...
  io:format("Cleanup (payload_size=~p)...~n", [PSize]),
  clear_table(Opts),
  timer:sleep(1000),
  Payload = list_to_binary([0 || _ <- lists:seq(1, PSize)]),
  Success = exec_test(Opts#{payload => Payload},
                      fun append_loop/3,
                      "append",
                      ["DB", "N", "NNodes", "PayloadSize", "BatchSize"],
                      [DB, N, NNodes, PSize, NItems]
                     ),
  case Success of
    true ->
      true;
...
append_test(UserOpts = #{db := _}) ->
  MinSize = maps:get(payload_size_min, UserOpts, 0),
  MaxSize = maps:get(payload_size_max, UserOpts, 10_000),
  Step = maps:get(payload_size_step, UserOpts, 1000),
  [append_test(UserOpts#{payload_size => PS}) || PS <- lists:seq(MinSize, MaxSize, Step)];
append_test(UserOpts) ->
  [?FUNCTION_NAME(maps:merge(UserOpts, maps:with([db, type], I))) || I <- test_dbs()].
#+end_src

*** 1000 workers
#+begin_src elisp :exports none
(my-run-in-docker "emqx_ds_otx_test:l(), emqx_ds_otx_test:append_test(#{n => 1000, repeats => 10, test_timeout => 300_000}), all_done.")
#+end_src

#+RESULTS:
: #<window 126 on *Async Shell Command*>

#+begin_src python :results file :exports result
a1kdl = df_append[((df_append['N'] == 1000) & (df_append['DB'] == 'dl'))]
histogram_vs(a1kdl, "1000 append workers, builtin_local, cutoff, 1ms flush")
#+end_src

#+RESULTS:
[[file:1000 append workers, builtin_local, cutoff, 1ms flush.hist.png]]

#+begin_src python :results file :exports result
boxplot_vs(a1kdl,"1000 append workers, 1ms flush",by='PayloadSize')
#+end_src


#+RESULTS:
[[file:1000 append workers, 1ms flush.bp.png]]

*** 1000 workers, raft w/ 1 replica
#+begin_src elisp :exports none
(my-run-in-docker "emqx_ds_otx_test:l(), emqx_ds_otx_test:append_test(#{n => 1000, repeats => 10, test_timeout => 300_000, db => dr}), all_done.")
#+end_src

#+RESULTS:
: #<window 115 on *Async Shell Command*>

#+begin_src python :results file :exports result
a1kdr1 = df_append[((df_append['N'] == 1000) & (df_append['DB'] == 'dr'))]
histogram_vs(a1kdr1, "1000 append workers, builtin_local, cutoff, raft, 1ms flush")
#+end_src

#+RESULTS:
[[file:1000 append workers, builtin_local, cutoff, raft, 1ms flush.hist.png]]

#+begin_src python :results file :exports result
boxplot_vs(a1kdr1,"1000 append workers, raft",by='PayloadSize')
#+end_src


#+RESULTS:
[[file:1000 append workers, raft.bp.png]]

*** 1000 workers, no beamformer, local

#+begin_src python :results file :exports result
a1kdnls = df_append[((df_append['N'] == 1000) & (df_append['DB'] == 'dlns'))]
histogram_vs(a1kdnls, "1000 append workers, builtin_local, no beamformer, 1ms flush")
#+end_src

#+RESULTS:
[[file:1000 append workers, builtin_local, no beamformer, 1ms flush.hist.png]]

#+begin_src python :results file :exports result
boxplot_vs(a1kdnls,"1000 append workers, no beamformer local",by='PayloadSize')
#+end_src


#+RESULTS:
[[file:1000 append workers, no beamformer local.bp.png]]
