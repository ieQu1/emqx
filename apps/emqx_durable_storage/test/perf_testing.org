...
=N= denotes the number of independent workers, =Nnodes= denotes the number of competing workers running on different nodes.

All tests run on a variety of DBs:

| DB   | Type | Backend        | Replicas | Reads  |
|------+------+----------------+----------+--------|
| d1l  | DS   | raft,rocksdb   |        1 | local  |
| d3l  | DS   | raft,rocksdb   |        3 | local  |
| d5L  | DS   | raft,rocksdb   |        5 | leader |
| dloc | DS   | local,rocksdb  |        1 | local  |
| md   | mria | disc_copies    |      2+3 | n/a    |
| mr   | mria | rocksdb_copies |      2+3 | n/a    |
| mloc | mria | rocksdb_copies |        1 | local  |
...
default_ds_settings() ->
  #{ store_ttv => true
   ...
   , transaction => #{flush_interval => 1, idle_flush_interval => 1, conflict_window => 10_000}
...
create_db(UserOpts = #{db := DB, type := ds}) ->
  ...
  Defaults = default_ds_settings(),
  Opts = emqx_utils_maps:deep_merge(Defaults, maps:remove(db, UserOpts)),
  case Opts of
    #{replication := true} ->
      multicall(fun() -> emqx_ds:open_db(DB, Opts#{backend => builtin_raft}) end);
    #{replication := false} ->
      emqx_ds:open_db(DB, Opts#{backend => builtin_local})
  end;
create_db(UserOpts = #{db := DB, type := mria}) ->
  Opts = maps:merge(
           #{type => ordered_set, storage => rocksdb_copies, rlog_shard => ?MRIA_SHARD},
           maps:without([db, rlog_shard, type, replication], UserOpts)
          ),
  case maps:get(replication, UserOpts, true) of
    true ->
      multicall(
        fun() ->
...
test_dbs() ->
  ...
  , #{type => ds, db => d1l, replication_factor => 1, reads => local_preferred, replication => false}
  %% Local DBs without replicas (for comparison)
  , #{type => ds, db => dloc, replication => false}
  , #{type => mria, db => mloc, replication => false}
  ].
...
#+begin_src elisp :exports none
(my-run-in-docker "emqx_ds_otx_test:l(), emqx_ds_otx_test:create_dbs().")
#+end_src


#+RESULTS:
: #<window 251 on *Async Shell Command*>
...
counter_test(UserOpts) ->
  [?FUNCTION_NAME(maps:merge(UserOpts, maps:with([db, type, replication], I))) || I <- test_dbs()].
#+end_src
...
A more optimized approach is when a process takes ownership over a record by writing a unique ID to a separate "guard" record, and then issues a transaction that

- Checks that the unique ID didn't change
- Overwrites the data
...
* Results, baseline
#+PROPERTY: header-args:python :session *python-baseline* :dir baseline

Copy CSVs from docker and parse it:

#+begin_src python :results discard :noweb yes :exports none
<<python-init>>
...
#+RESULTS:
: #<window 234 on *Async Shell Command*>
...
#+RESULTS:
[[file:baseline/1k naive counters, no sleep.hist.png]]
...
#+RESULTS:
[[file:baseline/1k naive counters, no sleep.bp.png]]
...
#+RESULTS:
[[file:baseline/10k naive counters, no sleep.hist.png]]
...
#+RESULTS:
[[file:baseline/10k naive counters, no sleep.bp.png]]
...
#+RESULTS:
: #<window 250 on *Async Shell Command*>
...
#+RESULTS:
[[file:baseline/10k naive counters, 30ms sleep.hist.png]]
...
#+RESULTS:
[[file:baseline/10k naive counters, 30ms sleep.bp.png]]
...
#+RESULTS:
[[file:baseline/10k owned counters, no conflicts, increment.hist.png]]
...
#+RESULTS:
[[file:baseline/10k owned counters, no conflicts, increment.bp.png]]
...
#+RESULTS:
[[file:baseline/10k owned counters, no conflicts, own.hist.png]]
...
exec_test(UserOpts, Fun, ExperimentName, ColumNames, MeasurementFields) ->
    ...
    AvailNodes = case maps:get(replication, UserOpts, true) of
                   true ->
                     [node() | nodes()];
                   false ->
                     [node()]
...
* Appendix B: Metrics and data processing
...
def histogram_vs_separate(df, title):
    ...
    bins = np.linspace(df['Val'].min(), df['Val'].max(), 100)
    # Plot it
    df.hist(column='Val', grid=True, by=mtypes, sharex=True, sharey=True, bins=bins, histtype='stepfilled', figsize = (10, 10))
    plt.tight_layout()
    fn = title + ".hist.png"
    plt.savefig(fn)
    return fn
